...
class UniswapLiquidityViewModel(
    val service: UniswapLiquidityService,
    val tradeService: UniswapLiquidityTradeService,
    ...
    val proceedParams: SendEvmData?
        get() {
            ...
            if (serviceState is UniswapLiquidityService.State.Ready) {
                val trade = (tradeService.state as? UniswapLiquidityTradeService.State.Ready)?.trade
                val swapInfo = SendEvmData.UniswapLiquidityInfo(
                    ...
                    priceImpact = trade?.let { formatter.liquidityPriceImpactViewItem(it) }
                )
                val warnings: List<Warning> = if (trade?.priceImpactLevel == UniswapLiquidityTradeService.PriceImpactLevel.Forbidden)
                    listOf(UniswapLiquidityModule.UniswapWarnings.PriceImpactWarning)
                else
                    listOf()
    ...
    private fun sync(serviceState: UniswapLiquidityService.State) {
        isLoadingLiveData.postValue(serviceState == UniswapLiquidityService.State.Loading)
        syncState()
    }
    ...
    private fun sync(tradeServiceState: UniswapLiquidityTradeService.State) {
        when (tradeServiceState) {
            is UniswapLiquidityTradeService.State.Ready -> {
    ...
    private fun checkPriceImpact(trade: UniswapLiquidityTradeService.Trade) {
    ...
    private fun syncState() {
        ...
        val approveAction = getApproveActionState(revokeAction, proceedAction)
        val approveStep = getApproveStep(revokeAction)
//        Log.e("longwen", "revokeAction=${revokeAction}, approveAction=${approveAction}, proceedAction=${proceedAction}")
    ...
    private fun getProceedActionState(revokeAction: SwapActionState): SwapActionState {
        return when {
            ...
            service.state is UniswapLiquidityService.State.Ready -> {
                SwapActionState.Enabled(Translator.getString(R.string.Swap_Proceed))
            }
            tradeService.state is UniswapLiquidityTradeService.State.Ready -> {
                when {
                    service.errors.any { it == SwapError.InsufficientBalance } -> {
    ...
    private fun getApproveActionState(revokeAction: SwapActionState, proceedAction: SwapActionState): SwapActionState {
        return when {
            revokeAction !is SwapActionState.Hidden || proceedAction is SwapActionState.Disabled-> {
                SwapActionState.Hidden
            }
            pendingAllowanceService.state == SwapPendingAllowanceState.Approving -> {
                SwapActionState.Disabled(Translator.getString(R.string.Swap_Approving))
            }
            tradeService.state is UniswapLiquidityTradeService.State.NotReady || service.errors.any { it == SwapError.InsufficientBalanceFrom } -> {
                SwapActionState.Hidden
            }
    ...
    private fun tradeViewItem(trade: UniswapLiquidityTradeService.Trade): TradeViewItem {
        return TradeViewItem(
            ...
            priceImpact = formatter.liquidityPriceImpactViewItem(trade, UniswapLiquidityTradeService.PriceImpactLevel.Warning),
            guaranteedAmount = formatter.liquidityGuaranteedAmountViewItem(
                trade.tradeData,
                tradeService.tokenFrom,
                tradeService.tokenTo
            )
    ...
    data class TradeViewItem(
        ...
        val priceImpact: UniswapLiquidityModule.PriceImpactViewItem? = null,
        val guaranteedAmount: UniswapLiquidityModule.GuaranteedAmountViewItem? = null
    )
