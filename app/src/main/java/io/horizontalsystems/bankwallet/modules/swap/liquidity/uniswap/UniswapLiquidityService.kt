...
class UniswapLiquidityService(
    private val dex: SwapMainModule.Dex,
    private val tradeService: UniswapLiquidityTradeService,
    ...
    private fun syncState() {
        ...
        when (val state = tradeService.state) {
            UniswapLiquidityTradeService.State.Loading -> {
                loading = true
            }
            ...
            is UniswapLiquidityTradeService.State.NotReady -> {
        ...
        tradeService.amountFrom?.let { amountFrom ->
            ...
            if (balanceFrom == null || balanceFrom < amountFrom) {
                allErrors.add(SwapError.InsufficientBalance)
            }
        }

        tradeService.amountTo?.let { amountTo ->
            val balanceFrom = balanceTo
            if (balanceFrom == null || balanceFrom < amountTo) {
                allErrors.add(SwapError.InsufficientBalance)
            }
        }
