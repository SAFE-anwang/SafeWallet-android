package io.horizontalsystems.bankwallet.modules.safe4.safe42wsafe.confirmation
...
class SendWsafe4ConfirmationFragment : BaseFragment() {

    private val logger = AppLogger("send-evm")
    private val sendWsafeViewModel by navGraphViewModels<Safe4ConvertSendViewModel>(R.id.sendWSafe4Fragment)
    ...
    private val sendEvmTransactionViewModel by navGraphViewModels<SendEvmTransactionViewModel>(R.id.sendWsafe4ConfirmationFragment) { vmFactory }
    private val nonceServiceViewModel by viewModels< SendEvmNonceViewModel> { vmFactory }
    private val feeViewModel by navGraphViewModels<EvmFeeCellViewModel>(R.id.sendWsafe4ConfirmationFragment) { vmFactory }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        sendEvmTransactionViewModel.sendSuccessLiveData.observe(viewLifecycleOwner) {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                findNavController().popBackStack(R.id.sendWSafe4Fragment, true)
            }, 1200)
        }
