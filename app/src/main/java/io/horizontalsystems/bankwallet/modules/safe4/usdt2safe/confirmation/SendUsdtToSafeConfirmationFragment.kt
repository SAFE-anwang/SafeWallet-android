package io.horizontalsystems.bankwallet.modules.safe4.usdt2safe.confirmation
...
class SendUsdtToSafeConfirmationFragment : BaseFragment() {

    private val logger = AppLogger("send-evm")
    private val sendWsafeViewModel by navGraphViewModels<SendUsdtToSafeViewModel>(R.id.sendUsdtToSafe4Fragment)
    ...
    private val sendEvmTransactionViewModel by navGraphViewModels<SendEvmTransactionViewModel>(R.id.sendUsdtToSafe4ConfirmationFragment) { vmFactory }
    private val nonceServiceViewModel by viewModels< SendEvmNonceViewModel> { vmFactory }
    private val feeViewModel by navGraphViewModels<EvmFeeCellViewModel>(R.id.sendUsdtToSafe4ConfirmationFragment) { vmFactory }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.toolbar.setOnMenuItemClickListener { item ->
            when (item.itemId) {
                R.id.menuClose -> {
                    findNavController().popBackStack(R.id.sendUsdtToSafe4Fragment, true)
                    true
                }
                else -> false
            }
