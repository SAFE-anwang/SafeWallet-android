...
import io.horizontalsystems.bankwallet.modules.swap.liquidity.confirmation.uniswap.UniswapLiquidityConfirmationModule
...
class UniswapLiquidityFragment : SwapBaseFragment() {

    private val vmFactory by lazy { UniswapLiquidityModule.Factory(dex) }
    private val uniswapViewModel by navGraphViewModels<UniswapLiquidityViewModel>(R.id.swapFragment) { vmFactory }
    private val allowanceViewModelFactory by lazy {
        UniswapLiquidityModule.AllowanceViewModelFactory(
    ...
    private val coinCardViewModelFactory by lazy {
        SwapMainModule.LiquidityCardViewModelFactory(
            this,
            dex,
            uniswapViewModel.service,
...
private fun UniswapScreen(
    viewModel: UniswapLiquidityViewModel,
    fromCoinCardViewModel: LiquidityCoinCardViewModel,
    toCoinCardViewModel: LiquidityCoinCardViewModel,
    ...
    val token by fromCoinCardViewModel.tokenCodeLiveData().observeAsState()
    ...
    ComposeAppTheme {
        Column(
            ...
            LiquidityCoinCardViewComposable(
                    ...
                    viewModel.proceedParams?.let { sendEvmData ->
                        token?.let {
                            navController.slideFromRight(
                                ...
                                UniswapLiquidityConfirmationModule.prepareParams(sendEvmData, it)
                            )
                        }
                    }
                }
