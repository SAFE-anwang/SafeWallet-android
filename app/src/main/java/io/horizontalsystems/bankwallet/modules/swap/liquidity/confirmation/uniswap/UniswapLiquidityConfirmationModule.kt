...
import io.horizontalsystems.bankwallet.modules.send.evm.SendLiquidityEvmData
...
object UniswapLiquidityConfirmationModule {
    ...
    class Factory(
        ...
        private val token by lazy { fromToken ?: App.marketKit.token(TokenQuery(blockchainType, TokenType.Native))!! }
    ...
    fun prepareParams(sendEvmData: SendEvmData, token: Token) = bundleOf(
        SendEvmModule.transactionDataKey to SendLiquidityEvmModule.TransactionDataParcelable(sendEvmData.transactionData),
        SendEvmModule.additionalInfoKey to sendEvmData.additionalInfo,
        SendEvmModule.transactionToken to token
    )
